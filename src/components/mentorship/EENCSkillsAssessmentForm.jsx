...
import {
    Card, Button, FormGroup, Input, Textarea, Modal
...
const calculateScores = (formData) => {
    ...
    const calculateSection = (items, sectionKey) => {
        items.forEach(item => {
            ...
            if (value && value !== 'na') {
                sectionScores[sectionKey].maxScore += 2; overallMax += 2;
                if (value === 'yes') { sectionScores[sectionKey].score += 2; overallScore += 2; } 
                else if (value === 'partial') { sectionScores[sectionKey].score += 1; overallScore += 1; }
            }
        });
    };
...
const ScoreCircle = ({ score, maxScore }) => {
    ...
    if (percentage >= 80) colorClasses = 'bg-green-100 text-green-700 border-green-200';
    else if (percentage >= 50) colorClasses = 'bg-yellow-100 text-yellow-700 border-yellow-200';

    return (
        <div className={`flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full border-2 text-xs font-bold ml-3 shadow-sm ${colorClasses}`} title={`${score}/${maxScore}`}>
            {percentage}%
        </div>
    );
};
...
const StickyOverallScore = ({ score, maxScore }) => {
    ...
    if (percentage >= 80) bgColor = 'bg-green-600';
    else if (percentage >= 50) bgColor = 'bg-yellow-500';
    else bgColor = 'bg-red-600';

    return (
        <div className={`fixed top-4 left-4 z-50 flex flex-col items-center justify-center p-3 w-20 h-20 rounded-lg ${bgColor} text-white shadow-2xl transition-all duration-300 transform hover:scale-105`} dir="rtl">
...
const SkillChecklistItem = ({ label, itemKey, value, onChange, showPartial = false, showNaOption = false }) => {
    ...
    if (showPartial) options.push(['جزئياً', 'partial', 'bg-yellow-500 border-yellow-500']);
    options.push(['لا', 'no', 'bg-red-600 border-red-600']);
                    ...
                    {items.map((item, idx) => {
                        ...
                        return (
                            <SkillChecklistItem
                                ...
                                showPartial={['dry_start_5sec', 'resus_check_chest_rise', 'normal_breastfeeding_guidance'].includes(item.key)}
                            />
                        );
                    })}
                </div>
                ...
                <div className="divide-y divide-slate-100 bg-white">
                    {visibleBaseItems.map((item) => (
                        <SkillChecklistItem
                            ...
                            value={formData.skills[item.key]}
                            onChange={handleSkillChange}
                            showPartial={item.key === 'resus_check_chest_rise'}
                        />
                    ))}
...
const EENCSkillsAssessmentForm = forwardRef((props, ref) => {
    const {
        ...
        setIsVisitReportModalOpen, draftCount, workerHistory = []
    } = props;

    const [formData, setFormData] = useState(() => existingSessionData ? rehydrateDraftData(existingSessionData) : getInitialFormData());
    ...
    const [currentVisitNumber, setCurrentVisitNumber] = useState(() => existingSessionData?.visitNumber ? existingSessionData.visitNumber : visitNumber);

    const eencHistoryStats = useMemo(() => {
        if (existingSessionData || !healthWorkerName || !facility?.id) return { maxVisitNum: 0, dateMap: {} };
        let maxVisitNum = 0; const dateMap = {};
        const validHistory = (workerHistory || []).filter(s => s.healthWorkerName === healthWorkerName || s.healthWorkerName === undefined);
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (matchingDateVisitNum !== null) newVisitNumber = matchingDateVisitNum;
            else { const absoluteMax = Math.max(maxVisitNum, localMaxVisitNum); newVisitNumber = absoluteMax + 1; }
            if (currentVisitNumber !== newVisitNumber) setCurrentVisitNumber(newVisitNumber);
            if (maxVisitNum > localMaxVisitNum) localStorage.setItem(offlineKey, maxVisitNum.toString());
        } catch (error) { console.error("Error calculating visit number:", error); }
    ...
    const allPropsRef = useRef({
        facility, healthWorkerName, user, existingSessionData, isSaving, isSavingDraft, setToast, healthWorkerJobTitle, onDraftCreated, healthWorkerTrainingDate, healthWorkerPhone, onExit, onSaveComplete, visitNumber, lastSessionDate, setIsMothersFormModalOpen, setIsDashboardModalOpen, setIsVisitReportModalOpen, draftCount
    });
    
    useEffect(() => {
        allPropsRef.current = { facility, healthWorkerName, user, existingSessionData, isSaving, isSavingDraft, setToast, healthWorkerJobTitle, onDraftCreated, healthWorkerTrainingDate, healthWorkerPhone, onExit, onSaveComplete, visitNumber, lastSessionDate, setIsMothersFormModalOpen, setIsDashboardModalOpen, setIsVisitReportModalOpen, draftCount };
    }, [ facility, healthWorkerName, user, existingSessionData, isSaving, isSavingDraft, setToast, healthWorkerJobTitle, onDraftCreated, healthWorkerTrainingDate, healthWorkerPhone, onExit, onSaveComplete, visitNumber, lastSessionDate, setIsMothersFormModalOpen, setIsDashboardModalOpen, setIsVisitReportModalOpen, draftCount ]);
    ...
    useEffect(() => {
        ...
        const updateRelevance = (items, isRelevant) => {
            items.forEach(item => {
                ...
                if (isRelevant && (skills[key] === 'na' || skills[key] === undefined)) {
                    skills[key] = ''; needsUpdate = true;
                } else if (!isRelevant && skills[key] !== 'na') {
                    skills[key] = 'na'; needsUpdate = true;
                }
            });
        };
        ...
        if (!isResus) {
            ...
            if (!baseComplete) {
                ...
                if (eenc_resus_breathed_normally === 'yes') {
                    updateRelevance([RESUSCITATION_ITEMS[10]], true); updateRelevance(RESUSCITATION_ITEMS.slice(11), false);
                    if (eenc_resus_has_pulse !== 'na') { newFormData.eenc_resus_has_pulse = 'na'; needsUpdate = true; }
                } else if (eenc_resus_breathed_normally === 'no') {
                    updateRelevance([RESUSCITATION_ITEMS[10]], false);
                    if (eenc_resus_has_pulse === 'yes') { updateRelevance([RESUSCITATION_ITEMS[11]], true); updateRelevance([RESUSCITATION_ITEMS[12]], false); } 
                    else if (eenc_resus_has_pulse === 'no') { updateRelevance([RESUSCITATION_ITEMS[11]], false); updateRelevance([RESUSCITATION_ITEMS[12]], true); } 
                    else { updateRelevance(RESUSCITATION_ITEMS.slice(11), false); }
                } else {
                    updateRelevance(RESUSCITATION_ITEMS.slice(10), false);
                    if (eenc_resus_has_pulse !== 'na') { newFormData.eenc_resus_has_pulse = 'na'; needsUpdate = true; }
                }
    ...
    const silentSaveDraft = useCallback(async () => {
        const { facility, healthWorkerName, user, isSaving, isSavingDraft, healthWorkerJobTitle, onDraftCreated } = allPropsRef.current;
    ...
    useImperativeHandle(ref, () => ({
        saveDraft: async () => { const { isSaving, isSavingDraft } = allPropsRef.current; if (isSaving || isSavingDraft) { return; } await silentSaveDraft(); },
    ...
    const handleSkillChange = (section, key, value) => { setFormData(prev => ({ ...prev, [section]: { ...prev[section], [key]: value, } })); };

    const handleSubmit = async (e, status = 'complete') => {
        if (e) e.preventDefault();
        const currentFormComplete = checkFormCompletion(formData);
        if (status === 'complete' && !currentFormComplete) { setToast({ show: true, message: "Please complete all fields before saving.", type: 'error', duration: 10000 }); return; }
        const savingDraft = status === 'draft';
        if (savingDraft) setIsSavingDraft(true); else setIsSaving(true);
        try {
            const calculatedScores = calculateScores(formData); const scoresPayload = {};
            for (const key in calculatedScores) { 
                if (calculatedScores[key]?.score !== undefined && calculatedScores[key]?.maxScore !== undefined) { scoresPayload[`${key}_score`] = calculatedScores[key].score; scoresPayload[`${key}_maxScore`] = calculatedScores[key].maxScore; }
            }
    ...
    return (
        <Card dir="rtl">
            ...
            <form onSubmit={(e) => handleSubmit(e, 'complete')}>
                <div className="p-6">
                    <div className="text-center mb-4"><h2 className="text-2xl font-bold text-sky-800">متابعة مهارات الرعاية الضرورية المبكرة (EENC)</h2></div>
                 ...
                 <div className="flex sm:hidden fixed bottom-16 left-0 right-0 z-20 h-16 justify-around items-center bg-gray-900 text-white border-t border-gray-700 shadow-lg" dir="rtl">
                    <Button type="button" variant="secondary" onClick={onExit} disabled={isSaving || isSavingDraft} size="sm"> إلغاء </Button>
                    <Button type="button" variant="outline" onClick={(e) => handleSubmit(e, 'draft')} disabled={isSaving || isSavingDraft || formData.eenc_breathing_status === 'na'} size="sm"> {isSavingDraft ? 'جاري...' : 'حفظ مسودة'} </Button>
                    <Button type="submit" disabled={isSaving || isSavingDraft || !isFormComplete} size="sm"> {isSaving ? 'جاري...' : 'حفظ وإكمال'} </Button>
                 </div>
            </form>
        </Card>
    );
