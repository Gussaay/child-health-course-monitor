...
import { 
    Button, Spinner, Card, PageHeader, EmptyState 
...
import { Download, X, Clock, CheckCircle, Trash2, Edit2, PlusCircle, Calendar } from 'lucide-react'; 
...
const generateAttendanceReportPdf = async (course, participants, columns) => {
    try {
        ...
        // Prepare headers: Date columns show MM-DD
        ...
        const tableData = participants.sort((a,b) => a.name.localeCompare(b.name)).map((p, index) => {
            ...
            // Add symbols for specific dates (Mark as raw P/A, we draw later)
        ...
        autoTable(doc, {
            ...
            columnStyles: {
                ...
                2: { cellWidth: 25, halign: 'left' }, // State
                3: { cellWidth: 35, halign: 'left' }, // Facility
                4: { cellWidth: 12, halign: 'center' }, // Group
                // Dynamic date columns default to auto/center
            },
            didParseCell: function (data) {
                ...
                // Is this a date column in the body?
                if (data.section === 'body' && data.column.index >= dateColumnStartIndex && data.column.index < dateColumnEndIndex) {
                    // Store the status in the cell but CLEAR the text so we can draw manually
            ...
            didDrawCell: function (data) {
                // Manually draw the Check/Cross shapes
...
export function AttendanceManagerView({ course, onClose }) {
    ...
    // Local state to hold manually added columns (before they have data)
    ...
    // Calculate columns: Union of existing data dates + manually added dates
    const sessionDates = useMemo(() => {
        const dates = new Set(manualDates);
        
        // Add dates found in participant data
            ...
            <div className="p-4 border-b flex flex-wrap justify-between items-center bg-gray-50 gap-4">
                ...
                <div className="flex gap-2">
                    <Button variant="primary" onClick={handleAddColumn} size="sm">
                        <PlusCircle className="w-4 h-4 mr-2" /> Add Session
                    </Button>
                ...
                {sessionDates.length === 0 ? (
                    ...
                    <table className="min-w-full divide-y divide-gray-200 border text-sm">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {participants.sort((a,b) => a.name.localeCompare(b.name)).map((p, index) => (
                                <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-2 py-1 text-xs text-center text-gray-500 sticky left-0 bg-white z-10 border-r">
                                        {index + 1}
                                    </td>
                                    <td className="px-3 py-1 text-sm font-medium text-gray-900 sticky left-10 bg-white z-10 border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                        {p.name}
                                    </td>
                                    <td className="px-3 py-1 text-xs text-gray-500 border-r">
                                        <div className="font-semibold truncate max-w-[180px]" title={p.health_facility}>{p.health_facility || '-'}</div>
                                        <div className="text-gray-400 flex items-center gap-1">
                                            <span>{p.state}</span>
                                            {p.group && <span className="px-1.5 py-0.5 rounded-full bg-gray-100 text-[10px] border">{p.group}</span>}
                                        </div>
                                    </td>
                                            ...
                                            <td key={date} className="px-2 py-1 text-center border-r">
                                                <div className="flex justify-center">
                                                    <input 
                                                        ...
                                                        className="h-5 w-5 text-green-600 focus:ring-green-500 border-gray-300 rounded cursor-pointer transition-all"
                                    ...
                                    <td className="px-3 py-1 text-center text-sm font-bold text-gray-700 border-l bg-gray-50 sticky right-0 z-10 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                        <span className={`px-2 py-0.5 rounded-full text-xs ${
                                            (p.attendance?.length || 0) === totalSessions ? 'bg-green-100 text-green-800' : 
                                            (p.attendance?.length || 0) < totalSessions / 2 ? 'bg-red-100 text-red-800' : 'bg-gray-200'
                                        }`}>
                        ...
                        <tfoot className="bg-gray-50 font-semibold sticky bottom-0 z-20 shadow-inner">
                            <tr>
                                ...
                                {dailyCounts.map((d, i) => (
                                    <td key={i} className="px-2 py-2 text-center text-xs text-gray-800 border-r">
                                        {d.count}
                                    </td>
                                ))}
...
// ... [PublicAttendanceView export remains the same] ...
export function PublicAttendanceView({ courseId }) {
    ...
    useEffect(() => {
        const checkDateAndLoadData = async () => {
            try {
                ...
                // Strict validation: The form works ONLY on the selected day
                if (today !== targetDate) {
                    setIsWrongDate(true);
                    setLoading(false);
                    return;
    ...
    const handleConfirmAttendance = async (participantId) => {
        try {
            ...
            // Add date to attendance array using arrayUnion (avoids duplicates)
            await updateDoc(participantRef, {
                attendance: arrayUnion(targetDate)
            });
            ...
            <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
                <Card className="max-w-md text-center p-8">
                    ...
                    <p className="text-gray-600">
                        This attendance form is only valid for <strong>{targetDate}</strong>.
                    </p>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Facility</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {participants.sort((a,b) => a.name.localeCompare(b.name)).map(p => {
                                    ...
                                    return (
                                        <tr key={p.id} className={isPresent ? "bg-green-50" : ""}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {p.health_facility || p.facilityName || '-'}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <Button 
                                                    ...
                                                    className={isPresent ? "opacity-50 cursor-not-allowed" : ""}
                                                >
                                                    {isPresent ? (
                                                        <>
