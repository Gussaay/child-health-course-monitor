  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto grid gap-6">
        ...
        {view === 'courses' && <CoursesView courses={courses.filter(c => c.course_type === activeCourseType)} onAdd={() => { setEditingCourse(null); setView('courseForm'); }} onOpen={(id) => { setSelectedCourseId(id); setSelectedParticipantId(null); setView('participants'); }} onEdit={handleEditCourse} onDelete={handleDeleteCourse} />}
        {view === 'courseForm' && <CourseForm courseType={activeCourseType} initialData={editingCourse} onCancel={() => { setEditingCourse(null); setView('courses'); }} onSave={async (payload) => { const id = await upsertCourse({ ...payload, id: editingCourse?.id, course_type: activeCourseType }); await refreshCourses(); setEditingCourse(null); setSelectedCourseId(id); setView('participants'); }} />}
        {view === 'participants' && selectedCourse && <ParticipantsView course={selectedCourse} participants={courseParticipants} onAdd={() => { setEditingParticipant(null); setView('participantForm'); }} onOpen={(pid) => { setSelectedParticipantId(pid); setView('observe'); }} onEdit={handleEditParticipant} onDelete={handleDeleteParticipant} />}
        {view === 'participantForm' && selectedCourse && <ParticipantForm course={selectedCourse} initialData={editingParticipant} onCancel={() => { setEditingParticipant(null); setView('participants'); }} onSave={async (p) => { await upsertParticipant({ ...p, id: editingParticipant?.id, courseId: selectedCourse.id }); await refreshParticipants(); setEditingParticipant(null); setView('participants'); }} />}
        {view === 'observe' && selectedCourse && selectedParticipant && <ObservationView course={selectedCourse} participant={selectedParticipant} participants={courseParticipants} onChangeParticipant={(id) => setSelectedParticipantId(id)} />}
        {view === 'reports' && selectedCourse && <ReportsView course={selectedCourse} participants={participants.filter(p => p.courseId === selectedCourse.id)} />}
      </div>
    </div>
  );
}
...
// --- Landing component (UPDATED to remove migration logic) ---
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {courses.map(c => (<tr key={c.id} className="border-b hover:bg-gray-50"><td className="py-2 pr-4">{c.state}</td><td className="py-2 pr-4">{c.locality}</td><td className="py-2 pr-4">{c.hall}</td><td className="py-2 pr-4">{c.coordinator}</td><td className="py-2 pr-4">{c.participants_count}</td><td className="py-2 pr-4">{Array.isArray(c.facilitators) ? c.facilitators.join(", ") : [c.facilitator1, c.facilitator2, c.facilitator3, c.facilitator4, c.facilitator5, c.facilitator6].filter(Boolean).join(", ")}</td><td className="py-2 pr-4"><div className="flex gap-2"><button className="px-3 py-1 rounded-lg border bg-blue-100" onClick={() => onOpen(c.id)}>Open</button><button className="px-3 py-1 rounded-lg border bg-yellow-100" onClick={() => onEdit(c)}>Edit</button><button className="px-3 py-1 rounded-lg border bg-red-100" onClick={() => onDelete(c.id)}>Delete</button></div></td></tr>))}
          </tbody>
        </table>
      </div>
    </section>
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <FieldA label="Facilitators"><div className="grid gap-2">{facilitators.map((v, i) => (<div key={i} className="flex gap-2"><input className="border rounded-lg p-2 w-full" value={v} onChange={(e) => setFac(i, e.target.value)} placeholder={`Facilitator ${i + 1}`} /><button type="button" className="px-3 py-2 rounded-lg border" onClick={() => removeFac(i)} disabled={facilitators.length <= 2}>−</button></div>))}<button type="button" className="px-3 py-2 rounded-lg border" onClick={addFac}>+ Add Facilitator</button></div></FieldA>
      </div>
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2"><select className="border rounded-lg p-2" value={groupFilter} onChange={(e) => setGroupFilter(e.target.value)}><option value="All">All Groups</option><option value="Group A">Group A</option><option value="Group B">Group B</option><option value="Group C">Group C</option><option value="Group D">Group D</option></select><button className="px-3 py-2 rounded-xl border" onClick={onAdd}>Add Participant</button></div>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {filtered.map(p => (<tr key={p.id} className="border-b hover:bg-gray-50"><td className="py-2 pr-4">{p.name}</td><td className="py-2 pr-4">{p.group}</td><td className="py-2 pr-4">{p.state}</td><td className="py-2 pr-4">{p.locality}</td><td className="py-2 pr-4">{p.center_name}</td><td className="py-2 pr-4">{p.phone}</td><td className="py-2 pr-4"><div className="flex gap-2"><button className="px-3 py-1 rounded-lg border bg-blue-100" onClick={() => onOpen(p.id)}>Monitoring</button><button className="px-3 py-1 rounded-lg border bg-yellow-100" onClick={() => onEdit(p)}>Edit</button><button className="px-3 py-1 rounded-lg border bg-red-100" onClick={() => onDelete(p.id)}>Delete</button></div></td></tr>))}
          </tbody>
        </table>
      </div>
    </section>
...
function ParticipantForm({ course, initialData, onCancel, onSave }) {
  ...
  const submit = () => {
    ...
    if (course.course_type === 'IMNCI') {
        p.trained_before = trainedIMNCI;
        p.num_other_providers = numProv;
        p.num_other_providers_imci = numProvIMCI;
        p.has_nutrition_service = hasNutri;
        p.has_immunization_service = hasImm;
        p.has_ors_room = hasORS;
        if (lastTrainIMNCI) {
          p.last_imci_training = lastTrainIMNCI;
        }
        if (!hasNutri && nearestNutri) {
          p.nearest_nutrition_center = nearestNutri;
        }
        if (!hasImm && nearestImm) {
          p.nearest_immunization_center = nearestImm;
        }
    }
    if (course.course_type === 'EENC') {
        p.trained_eenc_before = trainedEENC;
        p.other_staff_delivery_room = otherStaff;
        p.other_staff_eenc_trained = otherStaffTrained;
        p.has_nursery = hasNursery;
        p.has_immunization = hasImm;
        p.has_kangaroo_room = hasKangaroo;
        if (lastTrainEENC) {
          p.last_eenc_training = lastTrainEENC;
        }
    }
    onSave(p);
  };
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        {isImnci && (<> <FieldA label="Previously trained in IMCI?"><select className="border rounded-lg p-2 w-full" value={trainedIMNCI ? "yes" : "no"} onChange={(e) => setTrainedIMNCI(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA><FieldA label="Last IMCI training date"><input type="date" className="border rounded-lg p-2 w-full" value={lastTrainIMNCI} onChange={(e) => setLastTrainIMNCI(e.target.value)} /></FieldA><FieldA label="Is there a therapeutic nutrition service?"><select className="border rounded-lg p-2 w-full" value={hasNutri ? 'yes' : 'no'} onChange={e => setHasNutri(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA>{!hasNutri && (<FieldA label="If no, what is the nearest outpatient nutrition center?"><input className="border rounded-lg p-2 w-full" value={nearestNutri} onChange={e => setNearestNutri(e.target.value)} /></FieldA>)}<FieldA label="Is there an immunization service?"><select className="border rounded-lg p-2 w-full" value={hasImm ? 'yes' : 'no'} onChange={e => setHasImm(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA>{!hasImm && (<FieldA label="If no, what is the nearest immunization center?"><input className="border rounded-lg p-2 w-full" value={nearestImm} onChange={e => setNearestImm(e.target.value)} /></FieldA>)}<FieldA label="Is there an ORS corner service?"><select className="border rounded-lg p-2 w-full" value={hasORS ? 'yes' : 'no'} onChange={e => setHasORS(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA><FieldA label="Number of doctors and assistants at the health center"><input type="number" min="0" className="border rounded-lg p-2 w-full" value={numProv} onChange={(e) => setNumProv(Number(e.target.value || 0))} /></FieldA><FieldA label="Number of doctors/medical assistants trained in IMCI"><input type="number" min="0" className="border rounded-lg p-2 w-full" value={numProvIMCI} onChange={(e) => setNumProvIMCI(Number(e.target.value || 0))} /></FieldA></>)}
        {isEenc && (<> <FieldA label="Previously trained in EENC?"><select className="border rounded-lg p-2 w-full" value={trainedEENC ? "yes" : "no"} onChange={(e) => setTrainedEENC(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA><FieldA label="Last EENC training date"><input type="date" className="border rounded-lg p-2 w-full" value={lastTrainEENC} onChange={(e) => setLastTrainEENC(e.target.value)} /></FieldA><FieldA label="# of other providers in delivery room"><input type="number" min="0" className="border rounded-lg p-2 w-full" value={otherStaff} onChange={(e) => setOtherStaff(Number(e.target.value || 0))} /></FieldA><FieldA label="# of other providers trained in EENC"><input type="number" min="0" className="border rounded-lg p-2 w-full" value={otherStaffTrained} onChange={(e) => setOtherStaffTrained(Number(e.target.value || 0))} /></FieldA><FieldA label="Is there a nursery service?"><select className="border rounded-lg p-2 w-full" value={hasNursery ? 'yes' : 'no'} onChange={e => setHasNursery(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA><FieldA label="Is there an immunization service?"><select className="border rounded-lg p-2 w-full" value={hasImm ? 'yes' : 'no'} onChange={e => setHasImm(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA><FieldA label="Is there a Kangaroo care room?"><select className="border rounded-lg p-2 w-full" value={hasKangaroo ? 'yes' : 'no'} onChange={e => setHasKangaroo(e.target.value === 'yes')}><option value="no">No</option><option value="yes">Yes</option></select></FieldA></>)}
      </div>
...
function ObservationView({ course, participant, participants, onChangeParticipant }) {
  ...
  const submitCase = async () => {
    console.log("1. Starting submitCase function...");
    ...
    console.log("2. Data buffer contains:", entries);
    ...
    console.log("3. Attempting to save to Firestore. Case data:", caseData);
    
    try {
      await upsertCaseAndObservations(caseData, newObservations, editingCase?.id);
      
      console.log("4. Save to Firestore successful!");
      ...
      console.error("5. ERROR saving to Firestore:", error);
  ...
  return (
    <>
      <section className="grid gap-4">
        ...
        <Toolbar>
          <Block label="Select participant"><select className="border rounded-lg p-2 w-full" value={participant.id} onChange={(e) => onChangeParticipant(e.target.value)}>{participants.map(p => <option key={p.id} value={p.id}>{p.name} — {p.group}</option>)}</select></Block>
          {isImnci && <Block label="Setting"><select className="border rounded-lg p-2 w-full" value={setting} onChange={(e) => setSetting(e.target.value)} disabled={!!editingCase}><option value="OPD">Out-patient</option><option value="IPD">In-patient</option></select></Block>}
          {isImnci && <Block label="Age band"><select className="border rounded-lg p-2 w-full" value={age} onChange={(e) => setAge(e.target.value)} disabled={!!editingCase}><option value="GE2M_LE5Y">Sick child (2-59 months)</option><option value="LT2M">Sick young infant (0-59 days)</option></select></Block>}
          {isEenc && <Block label="EENC Scenario"><select className="border rounded-lg p-2 w-full" value={eencScenario} onChange={(e) => setEencScenario(e.target.value)} disabled={!!editingCase}><option value="breathing">Breathing Baby</option><option value="not_breathing">Not Breathing Baby</option></select></Block>}
        </Toolbar>
      </section>
      <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      ...
      {loading ? <div>Loading cases...</div> : <SubmittedCases course={course} participant={participant} observations={observations} cases={cases} onEditCase={handleEditCase} onDeleteCase={handleDeleteCase} />}
    </>
  );
}

function ImnciMonitoringGrid({ age, buffer, toggle }) {
  return (<table className="min-w-full text-sm"><thead><tr className="text-left border-b"><th className="py-2 pr-4 w-64">Item (Domain)</th><th className="py-2 pr-4 w-64">Classification</th><th className="py-2 pr-4 w-40">Action</th></tr></thead><tbody>{DOMAINS_BY_AGE_IMNCI[age].map(d => { const list = getClassListImnci(age, d); const title = DOMAIN_LABEL_IMNCI[d] || d; return (list && list.length ? list : ["(no items)"]).map((cls, i) => { const k = `${d}|${cls}`; const mark = buffer[k]; const rowClass = mark === 1 ? 'bg-green-50' : mark === 0 ? 'bg-pink-50' : ''; return (<tr key={`${d}-${i}`} className={`border-b ${rowClass}`}>{i === 0 && <td className="py-2 pr-4 align-top font-semibold text-gray-800" rowSpan={list.length}>{title}</td>}<td className="py-2 pr-4">{cls}</td><td className="py-2 pr-4"><div className="flex gap-2"><button className={`px-3 py-1 rounded-lg border ${mark === 1 ? 'bg-green-200' : ''}`} onClick={() => toggle(d, cls, 1)}>Correct</button><button className={`px-3 py-1 rounded-lg border ${mark === 0 ? 'bg-pink-200' : ''}`} onClick={() => toggle(d, cls, 0)}>Incorrect</button></div></td></tr>); }); })}</tbody></table>);
}
function EtatMonitoringGrid({ buffer, toggle }) {
  return (<table className="min-w-full text-sm"><thead><tr className="text-left border-b"><th className="py-2 pr-4 w-64">Domain</th><th className="py-2 pr-4 w-64">Skill</th><th className="py-2 pr-4 w-40">Action</th></tr></thead><tbody>{ETAT_DOMAINS.map(d => { const skills = SKILLS_ETAT[d]; const title = ETAT_DOMAIN_LABEL[d] || d; return skills.map((skill, i) => { const k = `${d}|${skill}`; const mark = buffer[k]; const rowClass = mark === 1 ? 'bg-green-50' : mark === 0 ? 'bg-pink-50' : ''; return (<tr key={`${d}-${i}`} className={`border-b ${rowClass}`}>{i === 0 && <td className="py-2 pr-4 align-top font-semibold text-gray-800" rowSpan={skills.length}>{title}</td>}<td className="py-2 pr-4">{skill}</td><td className="py-2 pr-4"><div className="flex gap-2"><button className={`px-3 py-1 rounded-lg border ${mark === 1 ? 'bg-green-200' : ''}`} onClick={() => toggle(d, skill, 1)}>Correct</button><button className={`px-3 py-1 rounded-lg border ${mark === 0 ? 'bg-pink-200' : ''}`} onClick={() => toggle(d, skill, 0)}>Incorrect</button></div></td></tr>); }); })}</tbody></table>);
}
...
function SubmittedCases({ course, participant, observations, cases, onEditCase, onDeleteCase }) {
  ...
  const exportCSV = () => {
    ...
    for (const x of participantRows) { lines.push([course.id, participant.id, safeCSV(participant.name), participant.group, x.encounter_date, x.day_of_course ?? '', x.setting, x.age_group ?? '', x.case_serial ?? '', x.case_age_months ?? '', x.domain, safeCSV(x.item_recorded), x.item_correct].join(',')); }
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      <div className="flex items-center justify-between"><h3 className="text-lg font-medium">Submitted cases for {participant.name}</h3><button className="px-3 py-2 rounded-xl border" onClick={exportCSV}>Export CSV</button></div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {caseRows.map((c, idx) => {
              const pct = calcPct(c.correct, c.total); return (
                <tr key={idx} className="border-b"><td className="py-2 pr-4">{c.date}</td><td className="py-2 pr-4">{c.day ?? ''}</td>{isImnci && <td className="py-2 pr-4">{c.setting}</td>}{(isImnci || isEenc) && <td className="py-2 pr-4">{getAgeLabel(c.age)}</td>}<td className="py-2 pr-4">{c.serial}</td><td className="py-2 pr-4">{c.total}</td><td className="py-2 pr-4">{c.correct}</td><td className={`py-2 pr-4 ${pctBgClass(pct)}`}>{fmtPct(pct)}</td><td className="py-2 pr-4"><div className="flex gap-2"><button className="px-3 py-1 rounded-lg border" onClick={() => onEditCase(c)}>Edit</button><button className="px-3 py-1 rounded-lg border" onClick={() => onDeleteCase(c)}>Delete</button></div></td></tr>
              );
            })}
          </tbody>
        </table>
...
function ReportsView({ course, participants }) {
  ...
  if (loading) { return <div className="bg-white rounded-2xl shadow p-5">Loading report data...</div>; }
  if (course.course_type === 'ETAT') { return <EtatReports course={course} participants={participants} allObs={allObs} allCases={allCases} />; }
  if (course.course_type === 'EENC') { return <EencReports course={course} participants={participants} allObs={allObs} allCases={allCases} />; }
...
function ImnciReports({ course, participants, allObs, allCases }) {
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      ...
      <div className="flex flex-wrap gap-3"><button className={`px-4 py-2 rounded-xl text-white ${tab === 'case' ? 'bg-blue-600' : 'bg-blue-500/80 hover:bg-blue-500'}`} onClick={() => setTab('case')}>Case Summary</button><button className={`px-4 py-2 rounded-xl text-white ${tab === 'class' ? 'bg-emerald-600' : 'bg-emerald-500/80 hover:bg-emerald-500'}`} onClick={() => setTab('class')}>Classification Summary</button><button className={`px-4 py-2 rounded-xl text-white ${tab === 'matrix' ? 'bg-indigo-600' : 'bg-indigo-500/80 hover:bg-indigo-500'}`} onClick={() => setTab('matrix')}>Detailed classification report</button></div>
      <div className="flex flex-wrap gap-4 items-center"><div className="flex items-center gap-2"><span className="text-sm font-semibold text-gray-800">Age group</span><select className="border rounded-lg p-2" value={age} onChange={(e) => setAge(e.target.value)}><option value="LT2M">0-2 months</option><option value="GE2M_LE5Y">2-59 months</option></select></div><div className="flex items-center gap-2"><span className="text-sm font-semibold text-gray-800">Setting</span><select className="border rounded-lg p-2" value={settingFilter} onChange={(e) => setSettingFilter(e.target.value)}><option value="All">All</option><option value="OPD">Out-patient</option><option value="IPD">In-patient</option></select></div></div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>{ids.map(id => {
                  ...
                  return (<tr key={id} className="border-b"><td className="py-2 pr-4">{r.name}</td><td className="py-2 pr-4">{inSeen}</td><td className="py-2 pr-4">{inCor}</td><td className={`py-2 pr-4 ${pctBgClass(pctIn)}`}>{fmtPct(pctIn)}</td><td className="py-2 pr-4">{outSeen}</td><td className="py-2 pr-4">{outCor}</td><td className={`py-2 pr-4 ${pctBgClass(pctOut)}`}>{fmtPct(pctOut)}</td><td className="py-2 pr-4">{tot}</td><td className={`py-2 pr-4 ${pctBgClass(pctAll)}`}>{fmtPct(pctAll)}</td></tr>);
                })}</tbody>
              </table>
            </div>
          </div>
          ...
          <div key={g} className="grid gap-2">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs">
                <thead><tr className="text-left border-b bg-gray-50"><th className="py-2 pr-4 w-80">Classification</th>{parts.map(p => (<th key={p.id} className="py-2 pr-4 whitespace-nowrap">{p.name}</th>))}<th className="py-2 pr-4">Total</th><th className="py-2 pr-4">Mean</th><th className="py-2 pr-4">Min</th><th className="py-2 pr-4">Max</th></tr></thead>
                <tbody>
                  {matrixRows.map((r, idx) => (<tr key={idx} className="border-b"><td className="py-2 pr-4">{r.item}</td>{r.counts.map((c, i) => (<td key={i} className="py-2 pr-4 text-center">{c}</td>))}<td className="py-2 pr-4 font-medium">{r.total}</td><td className="py-2 pr-4">{(Math.round(r.mean * 10) / 10).toFixed(1)}</td><td className="py-2 pr-4">{r.min}</td><td className="py-2 pr-4">{r.max}</td></tr>))}
                  <tr className="bg-emerald-50 font-medium"><td className="py-2 pr-4"># Correct (ticks)</td>{perPartCorrect.map((n, i) => (<td key={i} className="py-2 pr-4 text-center">{n}</td>))}<td className="py-2 pr-4" colSpan={4}></td></tr>
...
function EtatReports({ course, participants, allObs, allCases }) {
  ...
  function buildMatrixForGroup(groupKey) {
    const parts = participants.filter(p => p.group === groupKey).sort((a, b) => a.name.localeCompare(b.name));
    const matrixRows = [];
    for (const domain in SKILLS_ETAT) { for (const skill of SKILLS_ETAT[domain]) { const counts = parts.map(p => rows.filter(o => o.participant_id === p.id && o.item_recorded === skill).length); const total = counts.reduce((a, b) => a + b, 0); const mean = parts.length ? total / parts.length : 0; const min = counts.length ? Math.min(...counts) : 0; const max = counts.length ? Math.max(...counts) : 0; matrixRows.push({ domain: ETAT_DOMAIN_LABEL[domain], item: skill, counts, total, mean, min, max }); } }
    const perPartTotals = parts.map(p => rows.filter(o => o.participant_id === p.id).length);
    const perPartCorrect = parts.map(p => rows.filter(o => o.participant_id === p.id && o.item_correct === 1).length);
    const perPartPct = perPartTotals.map((den, i) => den ? (perPartCorrect[i] * 100) / den : NaN);
    return { parts, matrixRows, perPartTotals, perPartCorrect, perPartPct };
  ...
  return (
    <section className="bg-white rounded-2xl shadow p-5 grid gap-4">
      ...
      <div className="flex flex-wrap gap-3"><button className={`px-4 py-2 rounded-xl text-white ${tab === 'case' ? 'bg-blue-600' : 'bg-blue-500/80 hover:bg-blue-500'}`} onClick={() => setTab('case')}>Case Summary</button><button className={`px-4 py-2 rounded-xl text-white ${tab === 'matrix' ? 'bg-indigo-600' : 'bg-indigo-500/80 hover:bg-indigo-500'}`} onClick={() => setTab('matrix')}>Detailed Skill Report</button></div>
...
function ProgramLogo() { return (<svg width="42" height="42" viewBox="0 0 100 100" className="shrink-0"><rect width="100" height="100" rx="18" fill="#0a0a0a" /><g transform="translate(12,10)"><circle cx="38" cy="20" r="8" fill="#c63d2f" /><path d="M20 70 Q38 52 56 70 v10 H20z" fill="#4348a3" /><path d="M32 38 a10 10 0 0 1 12 0 l6 6 a20 20 0 0 1 6 14 v22 H20 V58 a20 20 0 0 1 6-14z" fill="#4a50b8" /></g></svg>); }
